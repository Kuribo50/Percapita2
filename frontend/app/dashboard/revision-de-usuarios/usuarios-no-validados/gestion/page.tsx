...
export default function GestionUsuariosNoValidadosPage() {
  ...
  const fetchUsuarios = useCallback(async () => {
    try {
      setLoading(true);
      const token = localStorage.getItem("authToken");
      const response = await fetch(`${API_URL}/api/usuarios-no-validados/`, {
        headers: {
      ...
      if (!hasInitializedFiltersRef.current && data.length) {
        const ultimo = data[0];
        if (ultimo.mes) {
          setSelectedMes(ultimo.mes);
        }
        if (ultimo.ano) {
          setSelectedAno(String(ultimo.ano));
        }
        hasInitializedFiltersRef.current = true;
      }
    } catch (error) {
      console.error("Error fetching usuarios:", error);
      toast.error("Error al cargar usuarios no validados");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  // Cargar usuarios y observaciones una sola vez
  useEffect(() => {
    fetchUsuarios();
  }, [fetchUsuarios]);
  ...
  const filtrarUsuarios = useCallback(
    (listaUsuarios: Usuario[], filtroEstado: string) => {
      return listaUsuarios.filter((usuario) => {
        ...
        const matchMes = !selectedMes || usuario.mes === selectedMes;
        const matchAno =
          !selectedAno || usuario.ano?.toString() === selectedAno;
        const centroComparacion = usuario.centro_actual || usuario.centro_salud;
        const matchCentro =
          !selectedCentro || centroComparacion === selectedCentro;
        const motivoPrincipal = obtenerMotivoPrincipal(usuario).toUpperCase();

        let matchEstado = false;
        if (filtroEstado === "todos") {
          matchEstado = true;
        } else {
          // Hacer match exacto con los motivos permitidos
          matchEstado = motivoPrincipal.includes(filtroEstado);
        }
  ...
  const filtrosAplicados = useMemo(() => {
    const resultado: Record<string, Usuario[]> = {
      ...
      "TRASLADO NEGATIVO": filtrarUsuarios(
        usuariosOrdenadosMemo,
        "TRASLADO NEGATIVO"
      ),
      "RECHAZADO PREVISIONAL": filtrarUsuarios(
        usuariosOrdenadosMemo,
        "RECHAZADO PREVISIONAL"
      ),
      FALLECIDO: filtrarUsuarios(usuariosOrdenadosMemo, "FALLECIDO"),
    };
    return resultado;
  ...
  const estadisticas = useMemo(() => {
    const stats: Record<string, number> = {
      ...
      "TRASLADO NEGATIVO": filtrosAplicados["TRASLADO NEGATIVO"]?.length || 0,
      "RECHAZADO PREVISIONAL":
        filtrosAplicados["RECHAZADO PREVISIONAL"]?.length || 0,
      FALLECIDO: filtrosAplicados["FALLECIDO"]?.length || 0,
    };
    return stats;
  }, [filtrosAplicados]);

  // Calcular usuarios paginados del tab actual
  ...
  // Obtener valores únicos para filtros - solo centros de nombreCentro
  const centrosUnicos = Array.from(
    new Set(
      usuarios
        .map((u) => u.centro_salud) // Solo de nombreCentro
                    ...
                    <td colSpan={8} className="py-12 text-center">
                      ...
                      <p className="text-sm text-gray-400 dark:text-gray-500">
                        {hayFiltros
                          ...
                          : "No hay usuarios no validados registrados"}
                      </p>
                    </td>
                  </tr>
                ) : (
      ...
      <div className="container mx-auto px-6 py-6 space-y-6">
          ...
          <div className="flex items-start justify-between">
            <div className="flex-1">
              ...
              <p className="text-gray-600 dark:text-gray-400 text-sm">
                Revisión de usuarios rechazados, fallecidos, traslados negativos
                y no validados
              </p>
            </div>
            <div className="flex items-center gap-3">
        ...
        <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-4 mb-6">
          ...
          <div className="group relative">
            <div className="absolute inset-0 bg-linear-to-r from-purple-400 to-purple-600 rounded-xl blur-lg opacity-50 group-hover:opacity-75 transition-opacity"></div>
            <div className="relative bg-white dark:bg-gray-800/50 rounded-xl p-4 border border-purple-200 dark:border-purple-900/30">
              <div className="flex items-center justify-between">
                <div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-blue-600 dark:text-blue-400 mt-1">
                    {estadisticas["TRASLADO NEGATIVO"] || 0}
                  </p>
                </div>
          ...
          {/* Rechazado Previsional */}
          <div className="group relative">
            <div className="absolute inset-0 bg-linear-to-r from-orange-400 to-orange-600 rounded-xl blur-lg opacity-50 group-hover:opacity-75 transition-opacity"></div>
            <div className="relative bg-white dark:bg-gray-800/50 rounded-xl p-4 border border-orange-200 dark:border-orange-900/30">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wide">
                    Previsional
                  ...
                  <p className="text-2xl font-bold text-orange-600 dark:text-orange-400 mt-1">
                    {estadisticas["RECHAZADO PREVISIONAL"] || 0}
                  </p>
                </div>
          ...
          {/* Fallecidos */}
          <div className="group relative">
            <div className="absolute inset-0 bg-linear-to-r from-gray-400 to-gray-600 rounded-xl blur-lg opacity-50 group-hover:opacity-75 transition-opacity"></div>
            <div className="relative bg-white dark:bg-gray-800/50 rounded-xl p-4 border border-gray-200 dark:border-gray-700/30">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wide">
                    Fallecidos
                  ...
                  <p className="text-2xl font-bold text-gray-600 dark:text-gray-400 mt-1">
                    {estadisticas["FALLECIDO"] || 0}
                  </p>
                </div>
                <div className="w-10 h-10 rounded-lg bg-gray-100 dark:bg-gray-700/20 flex items-center justify-center">
        ...
        <div className="bg-white dark:bg-gray-800/50 rounded-2xl border border-gray-200 dark:border-gray-700/50 shadow-lg">
          <Tabs
            ...
            className="w-full"
          >
            <TabsList className="grid w-full grid-cols-4 bg-gray-100 dark:bg-gray-900/50 p-1 rounded-t-2xl">
              {/* Tab Todos */}
              <TabsTrigger
                value="todos"
              ...
              {/* Tab Traslados Negativos */}
              <TabsTrigger
                value="TRASLADO NEGATIVO"
                className="flex items-center gap-2 data-[state=active]:bg-white dark:data-[state=active]:bg-gray-800 rounded-lg px-3 py-2 text-sm font-medium"
              >
                <Clock className="w-4 h-4" />
                Traslados ({estadisticas["TRASLADO NEGATIVO"] || 0})
              </TabsTrigger>

              {/* Tab Rechazado Previsional */}
              <TabsTrigger
                value="RECHAZADO PREVISIONAL"
                className="flex items-center gap-2 data-[state=active]:bg-white dark:data-[state=active]:bg-gray-800 rounded-lg px-3 py-2 text-sm font-medium"
              >
                <AlertCircle className="w-4 h-4" />
                Previsional ({estadisticas["RECHAZADO PREVISIONAL"] || 0})
              </TabsTrigger>
              ...
              >
                ...
                Fallecidos ({estadisticas["FALLECIDO"] || 0})
              </TabsTrigger>
            </TabsList>
            ...
            <div className="p-6">
              ...
              {/* Tab Traslados Negativos */}
              <TabsContent value="TRASLADO NEGATIVO" className="space-y-4 mt-0">
                {renderTablaUsuarios("TRASLADO NEGATIVO")}
              </TabsContent>
              ...
              >
                {renderTablaUsuarios("RECHAZADO PREVISIONAL")}
              </TabsContent>

              {/* Tab Fallecidos */}
              <TabsContent value="FALLECIDO" className="space-y-4 mt-0">
                {renderTablaUsuarios("FALLECIDO")}
              </TabsContent>
            </div>
          </Tabs>
        </div>
