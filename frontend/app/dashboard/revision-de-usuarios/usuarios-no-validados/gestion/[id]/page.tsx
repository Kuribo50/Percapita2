...
export default function UsuarioNoValidadoDetallePage() {
  ...
  const catalogos = useMemo(
    () => ({
      etnias: etnias.map((etnia) => ({
        id: etnia.id,
        nombre: etnia.nombre,
        codigo: etnia.codigo,
        activo: etnia.activo,
      })),
      nacionalidades: nacionalidades.map((nac) => ({
        id: nac.id,
        nombre: nac.nombre,
        codigo: nac.codigo,
        activo: nac.activo,
  ...
  const fetchUsuario = useCallback(async () => {
    ...
    try {
      ...
      // Primero intentar buscar en usuarios no validados
      let response = await fetch(
        `${API_URL}/api/usuarios-no-validados/${id}/`,
        {
          headers: {
      ...
      // Si no se encuentra, intentar buscar en nuevos usuarios (por si acaso)
      if (!response.ok && response.status === 404) {
        response = await fetch(`${API_URL}/api/nuevos-usuarios/${id}/`, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("authToken")}`,
          },
        });
  ...
  const handleDelete = useCallback(
    async (usuarioId: number) => {
      try {
        // Intentar eliminar de usuarios no validados
        let response = await fetch(
          `${API_URL}/api/usuarios-no-validados/${usuarioId}/`,
          {
            method: "DELETE",
            headers: {
              Authorization: `Bearer ${localStorage.getItem("authToken")}`,
        ...
        // Si no existe ahí, intentar de nuevos usuarios
        if (!response.ok && response.status === 404) {
          response = await fetch(
            `${API_URL}/api/nuevos-usuarios/${usuarioId}/`,
            {
              method: "DELETE",
              headers: {
                Authorization: `Bearer ${localStorage.getItem("authToken")}`,
        ...
        router.push(
          "/dashboard/revision-de-usuarios/usuarios-no-validados/gestion"
        );
      } catch (err) {
      ...
      <div className="container mx-auto px-6 py-8 space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-2xl font-semibold text-gray-900 dark:text-gray-100">
              Detalle de Usuario No Validado
            </h1>
          </div>
        </div>
        ...
        {isLoading ? (
          ...
          <UsuarioDetalleView
            ...
            onEdit={() => {
              toast.info("Edición no disponible para usuarios no validados");
            }}
          />
        ) : (
